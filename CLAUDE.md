...
- `scripts/heartbeat.mjs`: heartbeat parameters and verdicts as pure functions, so the fast gate can check them offline.
- `scripts/write-heartbeat.mjs`: stamps `heartbeat.json`. Only the scheduled run writes `last_scheduled_run`.
...
- Every `localStorage` call goes through the `STORAGE` registry. A raw string key is a whitelist violation and reds.
- **Liveness is proven by a timestamped trace, never by reading config.** The platform silently disables scheduled workflows on inactive repos, and the workflow file does not change when that happens, so "the cron is still in the file" stays green through exactly the failure it was meant to catch.
- **The heartbeat guard is structural, never a commit-message string.** The heartbeat is written only on `schedule` or an explicit dispatch input. The commit it pushes triggers a plain `push`, and that run never writes, so the loop terminates by construction. A string guard dies the moment someone edits a message template, and dying quietly is the whole problem.
- A manual stamp writes `last_manual_run` only. Freshness reads `last_scheduled_run`, so clicking a button by hand must not revive a dead cron.
- If a `run: |` block pipes into `tee`, that same block must set `pipefail`.
- Never write `cmd && echo ... && exit 0` under `set -e`. When `cmd` returns non-zero the whole chain exits non-zero and the step fails backwards. Use an explicit `if`.
- Report artifacts stay `report-*`, stdout logs stay `stdout-<slug>.log`.
- `AGENTS.md` and `CLAUDE.md` must stay byte-identical, and this file stays under 200 lines. Tighten wording or split docs, never raise the cap.
...
- A floor whose job is catching total freeze stays loose. `FPS_FLOOR` is 12 against a measured 38, and a second assertion reds if anyone tightens it past a third of the measured baseline. A floor that normal jitter can hit is a false-red factory, and false reds push people to change the product to please the ruler. The first person to break that guard was me: 13 x 3 = 39 > 38. The fix is the floor, not the guard.
- `MAX_HEARTBEAT_AGE_DAYS` is derived from the cron cadence plus a two-day margin, not picked by feel. Update `FPS_MEASURED_BASELINE` only from a real CI metric.
...
- `CARD.hDead`, `DEAD_LINES`, `DEAD_STRIP`, `SHOT_BAND`, `CARD_INNER_W` are one group. Adding a card line means growing `hDead`, which moves the card bottom, which pushes `SHOT_BAND.y0`. Guarded by four fast-gate assertions, two mutations, and one measured-font-metrics assertion in the browser gate.
...
- **The cron cadence and `MAX_HEARTBEAT_AGE_DAYS` are one pair.** Cadence days plus `HEARTBEAT_MARGIN_DAYS` must equal the cap, as an equality with a mutation behind it. Switching the cron to weekly without recomputing reds. Keep the cron minute off the hour, which is the queueing peak.
...
- README's referenced shot set must equal the actual `docs/shots/` file set, both directions, and README must mention a feature whose storage key, shortcut, or cron exists in the code. The witness is the code, not my memory.
...
- `npm run heartbeat`: stamp the heartbeat. Locally this only writes `last_manual_run`.

## What the gates prove
- Fast gate: deterministic stepping, non-mutation, collisions, scoring, bot survival across eight seeds, `bestOf` monotonicity and hostile-input sanitizing, coupling equalities, card-line fit and ordering, purity scan, storage-key whitelist, workflow set equality, pipefail scan, secret scan, rules-file limits, obligation freshness, README agreement with the shot set and the code. Plus the heartbeat block: a cron exists with a derivable cadence and no duplicates, the freshness cap matches that cadence, the heartbeat write is guarded by event identity rather than a commit message, the trace is fresh or still inside the first-run grace, and a manual stamp cannot revive a dead cron.
