...
for name in ['ready','play','dead']:
    ...
    svg=f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" data-kind="ci-screenshot"><title>Actual CI browser canvas capture: {name}</title><image width="{w}" height="{h}" href="data:image/png;base64,{base64.b64encode(b).decode()}"/></svg>\n'
    (R/('docs/shots/'+name+'.svg')).write_text(svg)
    shots.append({'name':name,'bytes':len(b),'sha256':sha,'canvas_sha':entry['canvasSha']})
provenance={'kind':'browser-ci-screenshots','capture':'native canvas PNG export from real running Chromium page; not CSS element bounding-box screenshot','source_sha':os.environ['GITHUB_SHA'],'run_id':os.environ['GITHUB_RUN_ID'],'run_attempt':os.environ['GITHUB_RUN_ATTEMPT'],'shots':shots,'audio_buffer':a}
...
for item in items:
    ...
    if item['id']==ids[0]:x.update(disposition='fulfilled',evidence='docs/shots/provenance.json: exact native browser canvas PNG embedded without redrawing')
...
s=s[:start]+'''这三张是 CI 中真实运行的 Chromium 页面所渲染的画布 PNG，SVG 只封装原始像素，不重绘、不缩放。采集使用画布原生 PNG 导出，不是受 CSS 小数边界影响的元素截图；来源提交、运行和每张图的 SHA-256 见 `docs/shots/provenance.json`。
...
for name in ['AGENTS.md','CLAUDE.md']:
    p=R/name;s=p.read_text();s=s.replace('README illustrations, not real screenshots.','real CI canvas PNG captures embedded in SVG, with provenance.json hashes.')
    ...
    p.write_text(s)
print('Published 3 native browser canvas captures; archived all 3 obligations with evidence; PCM controls:',json.dumps(a))
