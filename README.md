...
**而现在它们已经不一致了**：死亡弹窗多了一行最高分，真实高度从 172 涨到 200，
示意图还是旧的。这笔欠账登记在 `docs/OBLIGATIONS.json` 里，过期快闸门会红。
...
4. **翻越高度与关卡间距的耦合**。用引擎自己跑一遍量出“两根管道之间能爬多少”，
   不是拄计划值做乘法。两侧都会红。
5. **最高分真的活过了一次页面重载**。读一下内存里的变量证明不了持久化，所以那条断言
   真的 reload。而且先拿分再死,不然得到的是一条 `0 == 0` 的空断言。反向那侧：
   抹干存储后必须是 0，且差一局不得把最高分拉低。
6. **声音两个方向都会红**。开着敲空格必须真的创建并启动了一个音源节点，关了必须
   一个都不启动。还有一条前置断言守“这个 runner 里 AudioContext 存在”,不守的话
   上面两条会一起变成空断言。

另外每一类扫描器（纯度、`| tee` 必须同段有 pipefail、密钥形状、workflow 集合等于目录、
存储键白名单）都配了**变异体**，而且先断言“替换真的发生了”，否则得到的是一个
静默通过的假变异体。
...
帧率下限反方向：它**改松了**，从 20 降到 13。实测 38-42，20 只有两倍余量，
CI 上共享 CPU 拖一下就会假红，而假红会逗人去改产品迁就尺子。它只该抓彻底卡死。
另配了一条判词不同的断言：有人把它收紧到超过实测基线的三分之一，就红。

## 测不出来的

- **能不能真的听到声音。** 闸门只证明了音源节点被创建并启动，以及静音时一个都不启动。
  扬声器里究竟有没有声、好不好听，都验不了。已登记成带期限的义务，去试
  OfflineAudioContext 那条路，走不通就正式归到这一节。
- **好不好玩。** 全绿不代表好玩，这一条只能人验收。
- **手机触屏的手感。** 代码里有 `pointerdown`，CI 验不了真机手感。
- **隐私模式 / 禁用存储时的降级路径。** 代码里有 try，且降级事实会被记下来并暴露到
  诊断出口（报告里能看到），但 CI 里没有真的把存储禁掉跑一遍。
